...
pub fn load_parcel_locker_by_id(id: &str) -> Result<ParcelLocker, RedisError> {
    let mut con = connect();
    let key = make_parcel_locker_key(id);
    match con.hgetall::<String, HashMap<String, String>>(key) {
        Ok(pl_hm) => Ok(pl_hm.into()),
        Err(err) => Err(err)
    }
}
...
pub fn find_parcel_lockers_by_distance(longitude: f64, latitude: f64, radius: f64) -> Result<Vec<RadiusSearchResultSerializable>, RedisError> {
    let mut con = connect();
