...
template <typename T>
class SPSCRingBufferOptimizedMemoryOrdering : public SPSCRingBufferInterface<T> {
    struct Slot {
        // alignas(kCacheLineSize) T value; // не помогает
        T value;
    };
    public:
        explicit SPSCRingBufferOptimizedMemoryOrdering(const size_t capacity) : buffer_(capacity + 1){
        }
    ...
    private:
        ...
        alignas(kCacheLineSize) std::atomic<size_t> head_{0}; // Owned by consumer  // оптимизация-А
        size_t head_cached_{0};

        // char padding_2[kCacheLineSize]; // оптимизация-А
        alignas(kCacheLineSize) std::atomic<size_t> tail_{0}; // Owned by producer  // оптимизация-А
